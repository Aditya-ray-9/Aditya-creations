...
class SmallBasic :
    ...
    class textWindow :
        def write(String_prompt) :
            print(String_prompt)
        def writeline(String_promt) :
            print(String_promt)
        def readNumber() :
            try :
                inputed_int = int(input())
            except Exception as e :
                print("Error reading")
                print("please try again \n code exited with error code 1 ")
                pass
            return inputed_int
        def read() :
...
class games :
    ...
    class rock_paper_sissor :
        def main():
            options = ["Rock", "Paper", "Sissors", "Rock", "Paper", "Sissors", "Rock", "Paper"]
            bot_index = random.randint(0, int(len(options)) - 1)
            bot_options = options[bot_index]
            #for debugging
            # print(options[bot_index])

            condition  = True
            while condition == True :
                try :
                    user_options = int(input("write 1 for Rock 2 for Paper 3 for Sissors "))
                    condition = False
            ...
            if user_options == 1 :
                ...
                print("rerun the program and enter only 1, 2, 3 ")
                breakpoint
            print(f"you do {user}")
            print(f"bot do {bot_options}")

            #final result
            if user == bot_options :
                print("draw ")
                print("this game is a tie so please try again ")
                breakpoint
            elif user == "Paper" and bot_options == "Sissors" :
                print("bot wins you lose try again ")
                breakpoint
            elif user == "Sissors" and bot_options == "Paper" :
                print("you won the bot lose ")
                breakpoint
            elif user == "Rock" and bot_options == "Sissors" :
                print("you won the bot lose ")
                breakpoint
            elif user == "Sissors" and bot_options == "Rock" :
                print("you won the bot lose ")
                breakpoint
            elif user == "Rock" and bot_options == "Paper" :
                print("bot won you lose ")
                breakpoint
            else :
                pass
        def help() :
            print("the data is not avaliable ")
...
class hack :
    ...
    def encrypt_directory(directory_path, key):
        file_paths = []
        for root, _, files in operating_system.walk(directory_path):
            for file_name in files:
                file_path = operating_system.path.join(root, file_name)
...
class Alpha:
    ...
    def help() :
        try :
            ...
            sprint(f"sorry fail to open help file reason : \n               file not found")

def login_system(input1, input1_request, input2, input2_request, id) :

    number_file = open("num.txt", "rb+")
    number = int(number_file.read() or 0)

    def is_id_available(id_number):
        for root, dirs, files in operating_system.walk("."):
            for file in files:
                if file == "profile.txt":
                    with open(operating_system.path.join(root, file), "r") as f:
                        content = f.read()
    ...
    while True:
        ...
        while operating_system.path.exists(profile_dir):
            number += 1
            profile_dir = f"profile{number}"
        
        operating_system.mkdir(profile_dir)
        
        with open(operating_system.path.join(profile_dir, "profile.txt"), "w") as pro_file:
            pro_file.write(f"{input1_request}: {input1}\n{input2_request}: {input2}\nID: {id_number}")
...
class password :
    def generate(length, Symbol) :
        ...
        final_password = []
        while index <= length :
            final_password.extend(password[index])
            index+=1
        
        global len_Password_generator
        len_Password_generator = len(password)
    ...
    def info(debugging) :
        length_final = f"{len_Password_generator}"
        if debugging == True :
            return f"this is a password generator using this you can create a new and strong password of any length depend on your password size you give to the computer and this password generator can make 1 crore length password in just 114 minutes (2 hours, 6 minutes), Max length by debugging : {length_final}"
        else :
            return f"this is a password generator using this you can create a new and strong password of any length depend on your password size you give to the computer and this password generator can make 1 crore length password in just 114 minutes "
...
if __name__ == '__main__' :
    ...
    print(password.generate(length, True))
    print(password.generate_digits(length))
    print(password.info(True))
